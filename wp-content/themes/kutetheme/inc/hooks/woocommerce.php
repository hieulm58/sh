    ...
    function kt_get_tool_quickview(){
        if ( is_plugin_active( 'yith-woocommerce-quick-view/init.php' ) ) {
            ...
            if( $yith_wcqv_enable == 'yes' ){
                echo '<a href="#" class="search yith-wcqv-button" data-product_id="' . $product->get_id() . '">' . $label . '</a>';
            }
        } 
...
if( ! function_exists( 'kt_add_to_cart_message' ) ){
    function kt_add_to_cart_message(){
    	...
    	if (get_option('woocommerce_cart_redirect_after_add')=='yes') :
    		...
    		$message 	= sprintf('<a href="%s" class="button">%s</a> %s', get_permalink(wc_get_page_id('cart')), esc_attr__( 'View Cart &rarr;', 'kutetheme'), esc_attr__( 'Product successfully added to your cart.', 'kutetheme') );
    	endif;
    		return $message;
    }
}

add_filter( 'wc_add_to_cart_message_html', 'kt_add_to_cart_message', 10 );
